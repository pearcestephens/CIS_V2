...
try {
  ...
  if ((int)($hdr['status'] ?? 0) === 1) {
    po_jresp(true, ['po_id'=>$poId, 'status'=>'already_completed']);
  }
  ...
  foreach ($items as $it) {
    ...
    if ($live && po_table_exists($pdo,'inventory_adjust_requests')) {
      ...
      $j = $pdo->prepare('INSERT INTO inventory_adjust_requests
        ...
        VALUES (NULL,?,?,?,?,\'po-final\',\'pending\',?,?,NOW())
  ...
  $rid = po_create_receipt($pdo, $poId, $outletId, true, (int)$ctx['uid'], array_map(function($r){
    return [
  ...
  } elseif (po_has_column($pdo,'purchase_orders','completed_timestamp')) {
    $pdo->prepare('UPDATE purchase_orders SET completed_timestamp = NOW() WHERE purchase_order_id = ?')->execute([$poId]);
  }

  po_insert_event($pdo, $poId, 'submit.final', ['receipt_id'=>$rid,'enqueued'=>$enqueued], (int)$ctx['uid']);

  $data = ['po_id'=>$poId,'status'=>'completed','actions'=>['enqueued'=>$enqueued,'receipt_id'=>$rid]];
