...
try {
    ...
    $remain = is_array($in['outstanding_lines'] ?? null) ? array_values($in['outstanding_lines']) : [];
    $idk    = isset($in['idempotency_key']) ? (string)$in['idempotency_key'] : null;

    if ($tid <= 0 || !$remain) { http_response_code(400); echo json_encode(['ok'=>false,'error'=>['code'=>'bad_request','message'=>'transfer_pk and outstanding_lines[] required']]); exit; }

    $qc = new TransferQueueClient();
    $res = $qc->partial($tid, $remain, $idk);
