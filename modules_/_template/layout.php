...
/**
 * Standard CISV2 chrome: header -> sidebar -> container -> footer.
 * Expects local vars $meta and $content injected by ModuleTemplate::captureLayout().
 */

$title = (string)($meta['title'] ?? 'CISV2');
...
$jsAssets = $assets['js'] ?? [];
  ...
  <?php foreach ($cssAssets as $css): ?>
    <link rel="stylesheet" href="<?= htmlspecialchars((string)$css, ENT_QUOTES) ?>">
  <?php endforeach; ?>
...
<body>
      ...
      <header class="cis-header">
        <div class="container-fluid d-flex align-items-center justify-content-between py-3">
          <div>
            ...
            <h1 class="h4 mb-1"><?= htmlspecialchars($title, ENT_QUOTES) ?></h1>
  ...
  <?php foreach ($jsAssets as $js): ?>
    <script src="<?= htmlspecialchars((string)$js, ENT_QUOTES) ?>"></script>
  <?php endforeach; ?>
